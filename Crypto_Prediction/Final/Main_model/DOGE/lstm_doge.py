...
Original file is located at
    https://colab.research.google.com/drive/1kSA5f6JBmknvFxuafOc5Q-DR7Y3jZH-1
"""
...
# Fetch DOGE trade raw data
today = datetime.date.today()
DOGE_raw = yf.download('DOGE-USD', start=today-datetime.timedelta(days=700), end=today)
# Split the raw data into parts
DOGE_basic = DOGE_raw[['Adj Close', 'Open']]
DOGE_quant = DOGE_raw

plt.figure(figsize=(15, 8))
plt.title('DOGE Prices History')
plt.plot(DOGE_quant['Adj Close'])
plt.xlabel('Date')
plt.ylabel('Prices ($)')
...
def merge_sentimental_data(sentimental_data_file_path, DOGE_data):
  ...
  # 将new_data_adjusted添加到DOGE_quant作为新的一列
...
DOGE_data = DOGE_quant
merged_data = merge_sentimental_data(sentimental_data_file_path, DOGE_data)
...
y_test = DOGE_quant['Adj Close'].values[training_data_len:]
...
data = DOGE_quant.filter(['Adj Close'])
...
plt.savefig('./DOGE_Result.png', dpi=300, bbox_inches='tight')
plt.show()

file_path='./DOGE_prediction.csv'
